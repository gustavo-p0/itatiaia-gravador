          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="w-full max-w-sm rounded-lg p-4" style={{ background: 'linear-gradient(180deg, #d1d5db 0%, #9ca3af 50%, #6b7280 100%)', boxShadow: 'inset 0 1px 1px rgba(255,255,255,0.5), 0 4px 8px rgba(0,0,0,0.4), inset 0 0 20px rgba(0,0,0,0.1)' }}>
              <div className="relative">
                <div className="rounded-2xl p-6 mb-4" style={{ background: 'linear-gradient(180deg, #2d251b 0%, #1a1510 100%)', boxShadow: 'inset 0 0 40px rgba(0,0,0,0.8)', position: 'relative' }}>
                ...
                <div className="rounded p-3" style={{ background: 'linear-gradient(135deg, #fef3c7 0%, #d4a84b 100%)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.5), 0 2px 4px rgba(0,0,0,0.3)', border: '1px solid #a07020' }}>
                  <AudioPlayer src={audioUrl} fileId={currentFile?.id} onPrev={handlePrev} onNext={handleNext} hasPrev={currentIndex > 0} hasNext={currentIndex < files.length - 1} onEnded={handleEnded} onClear={handleClear} onSongRecognized={setCurrentSong} loadingAudio={loadingAudio} onAudioReady={() => setLoadingAudio(false)} onLoadingStart={() => setLoadingAudio(true)} onPlayingChange={setPlaying} />
                </div>
              ...
              <details className="text-xs mt-4 text-center w-full max-w-sm mx-auto" style={{ color: '#4a3020' }}>
                <summary className="cursor-pointer hover:underline">Aviso legal e titularidade</summary>
                <div className="text-left mt-2 p-3 rounded" style={{ background: '#fef3c7' }}>
                  ...
                  <p className="mb-2" style={{ color: '#5a4020' }}>Não há intenção de concorrer com os canais oficiais, streaming ou qualquer outro meio do titular.</p>
                  <p className="mb-2" style={{ color: '#5a4020' }}>As expressões "Dona da Noite", "Itatiaia", logotipos e demais conteúdos são de titularidade de terceiros.</p>
                  <p style={{ color: '#5a4020' }}>As informações têm caráter meramente informativo.</p>
                </div>
              </details>
            </div>
          </div>
